	...
	switch(state)
		if(0)
			if(istype(P, /obj/item/weapon/wrench))
				...
				if(do_after(user, 20))
					user << "lue You wrench the frame into place."
					src.anchored = 1
					src.state = 1
			if(istype(P, /obj/item/weapon/weldingtool))
				playsound(src.loc, 'sound/items/Welder.ogg', 50, 1)
				if(do_after(user, 20))
					user << "lue You deconstruct the frame."
		...
		if(1)
			if(istype(P, /obj/item/weapon/wrench))
				playsound(src.loc, 'sound/items/Ratchet.ogg', 50, 1)
				if(do_after(user, 20))
					user << "lue You unfasten the frame."
					src.anchored = 0
					src.state = 0
			if(istype(P, /obj/item/weapon/motherboard) && !mainboard)
				playsound(src.loc, 'sound/items/Deconstruct.ogg', 50, 1)
				user << "lue You place the mainboard inside the frame."
				src.icon_state = "1"
				src.mainboard = P
				user.drop_item()
				P.loc = src
			if(istype(P, /obj/item/weapon/screwdriver) && mainboard)
				...
				user << "lue You screw the mainboard into place."
				src.state = 2
				src.icon_state = "2"
			if(istype(P, /obj/item/weapon/crowbar) && mainboard)
				playsound(src.loc, 'sound/items/Crowbar.ogg', 50, 1)
				user << "lue You remove the mainboard."
				src.state = 1
				src.icon_state = "0"
		...
		if(2)
			if(istype(P, /obj/item/weapon/screwdriver) && mainboard && (!peripherals.len))
				playsound(src.loc, 'sound/items/Screwdriver.ogg', 50, 1)
				user << "lue You unfasten the mainboard."
				src.state = 1
				src.icon_state = "1"
			...
			if(istype(P, /obj/item/stack/cable_coil))
				if(P:amount >= 5)
					...
					if(do_after(user, 20))
						P:amount -= 5
						if(!P:amount) del(P)
						user << "lue You add cables to the frame."
						src.state = 3
						src.icon_state = "3"
		if(3)
			if(istype(P, /obj/item/weapon/wirecutters))
				playsound(src.loc, 'sound/items/Wirecutter.ogg', 50, 1)
				user << "lue You remove the cables."
				src.state = 2
				src.icon_state = "2"
			...
			if(istype(P, /obj/item/weapon/disk/data/fixed_disk) && !src.hd)
				...
				src.hd = P
				P.loc = src
				user << "lue You connect the drive to the cabling."

			if(istype(P, /obj/item/weapon/crowbar) && src.hd)
				playsound(src.loc, 'sound/items/Crowbar.ogg', 50, 1)
				user << "lue You remove the hard drive."
				src.hd.loc = src.loc
				src.hd = null
			...
			if(istype(P, /obj/item/stack/sheet/glass))
				if(P:amount >= 2)
					...
					if(do_after(user, 20))
						P:use(2)
						user << "lue You put in the glass panel."
						src.state = 4
						src.icon_state = "4"
		if(4)
			if(istype(P, /obj/item/weapon/crowbar))
				playsound(src.loc, 'sound/items/Crowbar.ogg', 50, 1)
				user << "lue You remove the glass panel."
				src.state = 3
				src.icon_state = "3"
				new /obj/item/stack/sheet/glass( src.loc, 2 )
			if(istype(P, /obj/item/weapon/screwdriver))
				playsound(src.loc, 'sound/items/Screwdriver.ogg', 50, 1)
				user << "lue You connect the monitor."
