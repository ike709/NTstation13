...
/obj/effect/decal/ash/attack_hand(mob/user as mob)
	usr << "lue The ashes slip through your fingers."
	del(src)
	return
...
/obj/item/assembly/time_ignite/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if (src.status)
		user.show_message("lue The timer is now secured!", 1)
	else
		user.show_message("lue The timer is now unsecured!", 1)
...
/obj/item/assembly/anal_ignite/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if (( istype(W, /obj/item/weapon/screwdriver) ))
		src.status = !( src.status )
		if (src.status)
			user.show_message("lue The analyzer is now secured!", 1)
		else
			user.show_message("lue The analyzer is now unsecured!", 1)
		src.part2.secured = src.status
		src.add_fingerprint(user)
...
/obj/item/assembly/a_i_a/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if (( istype(W, /obj/item/weapon/screwdriver) ))
		...
		src.status = !( src.status )
		if (src.status)
			user.show_message("lue The armor is now secured!", 1)
		else
			user.show_message("lue The armor is now unsecured!", 1)
		src.add_fingerprint(user)
...
/obj/item/assembly/rad_time/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	src.status = !( src.status )
	if (src.status)
		user.show_message("lue The signaler is now secured!", 1)
	else
		user.show_message("lue The signaler is now unsecured!", 1)
...
/obj/item/assembly/rad_prox/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	src.status = !( src.status )
	if (src.status)
		user.show_message("lue The proximity sensor is now secured!", 1)
	else
		user.show_message("lue The proximity sensor is now unsecured!", 1)
	src.part1.b_stat = !( src.status )
	src.add_fingerprint(user)
	return
...
/obj/item/assembly/rad_infra/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	src.status = !( src.status )
	if (src.status)
		user.show_message("lue The infrared laser is now secured!", 1)
	else
		user.show_message("lue The infrared laser is now unsecured!", 1)
	src.part1.b_stat = !( src.status )
	src.add_fingerprint(user)
	return
...
/obj/item/assembly/prox_ignite/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	src.status = !( src.status )
	if (src.status)
		user.show_message("lue The proximity sensor is now secured! The igniter now works!", 1)
	else
		user.show_message("lue The proximity sensor is now unsecured! The igniter will not work.", 1)
	src.part2.secured = src.status
	src.add_fingerprint(user)
	return
...
/obj/item/assembly/rad_ignite/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	src.status = !( src.status )
	if (src.status)
		user.show_message("lue The radio is now secured! The igniter now works!", 1)
	else
		user.show_message("lue The radio is now unsecured! The igniter will not work.", 1)
	src.part2.secured = src.status
	src.part1.b_stat = !( src.status )
	src.add_fingerprint(user)
	return
...
/obj/item/assembly/m_i_ptank/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if (!( src.status ))
		...
		user.show_message("lue A pressure hole has been bored to the plasma tank valve. The plasma tank can now be ignited.", 1)
	else
		src.status = 0
		bombers += "[key_name(user)] unwelded a prox bomb. Temp: [src.part3.air_contents.temperature-T0C]"
		user << "lue The hole has been closed."
	src.part2.secured = src.status
	src.add_fingerprint(user)
	return
