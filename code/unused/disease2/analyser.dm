	...
	if(istype(I,/obj/item/weapon/virusdish))
		...
		if(!dish)
			...
			for(var/mob/M in viewers(src))
				if(M == user)	continue
				M.show_message("lue [user.name] inserts the [dish.name] in the [src.name]", 3)
	...
	if(scanning)
		...
		if(scanning == 0)
			...
			for(var/mob/O in hearers(src, null))
				O.show_message("\icon[src] lue The [src.name] prints a sheet of paper", 3)
		...
		if(dish.virus2 && dish.growth > 50)
			...
			spawn(25)
				...
				dish = null
				for(var/mob/M in viewers(src))
					M.show_message("\icon[src] lue The [src.name] buzzes", 2)
				pause = 0
