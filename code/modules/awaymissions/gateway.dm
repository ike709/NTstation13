...
/obj/machinery/gateway/centerstation/attackby(obj/item/device/W as obj, mob/user as mob)
	if(istype(W,/obj/item/device/multitool))
		user << "lack The gate is already calibrated, there is no work for you to do here."
		return
	...
	if(istype(M, /mob/living/carbon))
		for(var/obj/item/weapon/implant/exile/E in M)//Checking that there is an exile implant in the contents
			if(E.imp_in == M)//Checking that it's actually implanted vs just in their pocket
				M << "lack The station gate has detected your exile implant and is blocking your entry."
				return
...
/obj/machinery/gateway/centeraway/attackby(obj/item/device/W as obj, mob/user as mob)
	if(istype(W,/obj/item/device/multitool))
		if(calibrated)
			...
			user << "lue <b>Recalibration successful!</b>: lack This gate's systems have been fine tuned.  Travel to this gate will now be on target."
			calibrated = 1
			return