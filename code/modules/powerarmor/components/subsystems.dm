...
/obj/item/weapon/powerarmor/servos/toggle(sudden = 0)
	switch(parent.active)
		if(1)
			if(!sudden)
				usr << "lue Movement assist servos disengaged."
			parent.slowdown += toggleslowdown
		if(0)
			usr << "lue Movement assist servos engaged."
			parent.slowdown -= toggleslowdown
...
/obj/item/weapon/powerarmor/medinj/toggle(sudden = 0)
	switch(parent.active)
		if(1)
			if(!sudden)
				usr << "lue Medical injector disengaged."
		if(0)
			usr << "lue Medical injector engaged."
...
/obj/item/weapon/powerarmor/autoext/toggle(sudden = 0)
	switch(parent.active)
		if(1)
			if(!sudden)
				usr << "lue Automatic fire extinguisher disengaged."
		if(0)
			usr << "lue Automatic fire extinguisher engaged."
	...
	switch(parent.active)
		if(1)
			if(active)
				toggle_grip(sudden)
			if(!sudden)
				usr << "lue Magnetic grip modules disengaged."
		if(0)
			usr << "lue Magnetic grip modules engaged."
...
/obj/item/weapon/powerarmor/grip/magnetic/proc/toggle_grip(sudden = 0)
	...
	if(active)
		if(!sudden)
			user << "lue Magnetic grip modules deactivated."
		parent.slowdown -= toggleslowdown
		parent.shoes.flags &= ~NOSLIP
	else
		user << "lue Magnetic grip modules activated."
		parent.slowdown += toggleslowdown
		parent.shoes.flags |= NOSLIP
...
/obj/item/weapon/powerarmor/orecollector/toggle(sudden = 0)
	switch(parent.active)
		if(1)
			if(!sudden)
				usr << "lue Ore collector module disengaged."
		if(0)
			usr << "lue Ore collector module engaged."
...
/obj/item/weapon/powerarmor/orecollector/user_click(var/atom/A, var/proximity, var/mob/living/carbon/human/user, var/intent)
	if(proximity && contents.len && istype(A, /obj/structure/ore_box))
		...
		for(var/obj/item/weapon/ore/O in contents)
			O.loc = box
		user << "lue You empty the ore collector into the box."
		return 1
