...
/obj/item/clothing/mask/facehugger/examine()
	...
	switch(stat)
		if(DEAD,UNCONSCIOUS)
			usr << "\red [src] is not moving."
		if(CONSCIOUS)
			usr << "\red [src] seems to be active."
	if (sterile)
		usr << "\red It looks like the proboscis has been removed."
	return
...
/obj/item/clothing/mask/facehugger/proc/Attach(M as mob)
	...
	L.visible_message("\red [src] leaps at [L]'s face!")

	if(ishuman(L))
		var/mob/living/carbon/human/H = L
		if(H.head && H.head.flags & HEADCOVERSMOUTH)
			H.visible_message("\red [src] smashes against [H]'s [H.head]!")
			Die()
			return 0
	...
	if(iscarbon(M))
		...
		if(target.wear_mask)
			...
			target.visible_message("\red [src] tears [W] off of [target]'s face!")
...
/obj/item/clothing/mask/facehugger/proc/Impregnate(mob/living/target as mob)
	...
	if(!sterile)
		...
		target.visible_message("\red [src] falls limp after violating [target]'s face!")
		...
		target.visible_message("\red [src] violates [target]'s face!")
	return
...
/*		RemoveActiveIndicators()	*/
	...
	src.visible_message("\red [src] curls up into a ball!")
