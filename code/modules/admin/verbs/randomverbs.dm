	...
	if(usr)
		if (usr.client)
			if(usr.client.holder)
				M << "old You hear a voice in your head... [msg]"

	log_admin("SubtlePM: [key_name(usr)] -> [key_name(M)] : [msg]")
	message_admins("lue old SubtleMessage: [key_name_admin(usr)] -> [key_name_admin(M)] : [msg]", 1)
...
/client/proc/cmd_admin_world_narrate()
	...
	message_admins("lue old GlobalNarrate: [key_name_admin(usr)] : [msg]<BR>", 1)
...
/client/proc/cmd_admin_direct_narrate(var/mob/M)
	...
	message_admins("lue old DirectNarrate: [key_name(usr)] to ([M.name]/[M.key]): [msg]<BR>", 1)
...
/client/proc/cmd_admin_godmode(mob/M as mob in mob_list)
	...
	usr << "lue Toggled [(M.status_flags & GODMODE) ? "ON" : "OFF"]"
...
/proc/create_xeno(ckey)
	...
	new_xeno.ckey = ckey
	message_admins("lue [key_name_admin(usr)] has spawned [ckey] as a filthy xeno [alien_caste].", 1)
	return 1
	...
	if(G_found.mind && !G_found.mind.active)	//mind isn't currently in use by someone/something
		...
		if(G_found.mind.assigned_role=="Alien")
			if(alert("This character appears to have been an alien. Would you like to respawn them as such?",,"Yes","No")=="Yes")
				...
				message_admins("lue [key_name_admin(usr)] has respawned [new_xeno.key] as a filthy xeno.", 1)
		...
		//check if they were a monkey
			...
			if(alert("This character appears to have been a monkey. Would you like to respawn them as such?",,"Yes","No")=="Yes")
				...
				message_admins("lue [key_name_admin(usr)] has respawned [new_monkey.key] as a filthy xeno.", 1)
				return	//all done. The ghost is auto-deleted
...
*/
	...
	message_admins("lue [admin] has respawned [player_key] as [new_character.real_name].", 1)
...
/client/proc/cmd_admin_gib_self()
	...
	if(confirm == "Yes")
		...
		message_admins("lue [key_name_admin(usr)] used gibself.", 1)
...
/client/proc/toggle_view_range()
	...
	//message_admins("lue [key_name_admin(usr)] changed their view range to [view].", 1)	//why? removed by order of XSI
...
/client/proc/admin_call_shuttle()
	...
	message_admins("lue [key_name_admin(usr)] admin-called the emergency shuttle.", 1)
	return
...
/client/proc/admin_cancel_shuttle()
	...
	message_admins("lue [key_name_admin(usr)] admin-recalled the emergency shuttle.", 1)
...
/client/proc/everyone_random()
	...
	if(notifyplayers == "Yes")
		world << "lue <b>Admin [usr.key] has forced the players to have completely random identities!"
