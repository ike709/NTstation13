/client/proc/triple_ai()
	...
	if(job_master && ticker)
		...
		if(ticker.triai)
			...
			message_admins("lue [key_name_admin(usr)] has toggled off triple AIs at round start.", 1)
		else
			ticker.triai = 1
			usr << "There will be an AI Triumvirate at round start."
			message_admins("lue [key_name_admin(usr)] has toggled on triple AIs at round start.", 1)
	return
