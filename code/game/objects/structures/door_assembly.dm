...
/obj/structure/door_assembly/attackby(obj/item/W as obj, mob/user as mob)
	...
	//	or you will get broken icons.
		...
		if(WT.can_use(user))
			...
			user << "lue You change the paintjob on the airlock assembly."
		...
		if(WT.remove_fuel(0,user))
			...
			if(do_after(user, 40))
				...
				user << "lue You've dissasembled the airlock assembly."
			...
			user << "lue You need more welding fuel to dissassemble the airlock assembly."
			return
		...
		if(do_after(user, 40))
			if(!src) return
			user << "lue You've secured the airlock assembly."
		...
		if(do_after(user, 40))
			if(!src) return
			user << "lue You've unsecured the airlock assembly."
		...
		if(do_after(user, 40))
			...
			coil.use(1)
			src.state = 1
			user << "lue You've wired the airlock assembly."
		...
		if(do_after(user, 40))
			if(!src) return
			user << "lue You've cut the wires from the airlock assembly."
		...
		if(G)
			if(G.amount>=1)
				if(G.type == /obj/item/stack/sheet/rglass)
					...
					if(do_after(user, 40))
						user << "lue You've installed reinforced glass windows into the airlock assembly."
						G.use(1)
						mineral = "glass"
					...
					if(G.amount>=2)
						...
						user.visible_message("[user] adds [G.name] to the airlock assembly.", "You start to install [G.name] into the airlock assembly.")
						if(do_after(user, 40))
							user << "lue You've installed [M] plating into the airlock assembly."
							G.use(2)
							mineral = "[M]"
		...
		playsound(src.loc, 'sound/items/Screwdriver.ogg', 100, 1)
		user << "lue You start finishing the airlock."

		if(do_after(user, 40))
			if(!src) return
			user << "lue You've finished the airlock."
