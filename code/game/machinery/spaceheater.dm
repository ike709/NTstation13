/obj/machinery/space_heater
			...
			if(open)
				if(cell)
					...
					if(istype(C))
						...
						user.visible_message("lue [user] inserts a power cell into [src].", "lue You insert the power cell into [src].")
			else
				user << "The hatch must be open to insert a power cell."
				return
		else if(istype(I, /obj/item/weapon/screwdriver))
			open = !open
			user.visible_message("lue [user] [open ? "opens" : "closes"] the hatch on the [src].", "lue You [open ? "open" : "close"] the hatch on the [src].")
			update_icon()
	...
	attack_hand(mob/user as mob)
		...
		if(open)
			...
			on = !on
			user.visible_message("lue [user] switches [on ? "on" : "off"] the [src].","lue You switch [on ? "on" : "off"] the [src].")
			update_icon()
		return
		...
		switch(href_list["op"])
			...
			if("cellremove")
				if(open && cell && !usr.get_active_hand())
					...
					cell = null
					usr.visible_message("lue [usr] removes the power cell from \the [src].", "lue You remove the power cell from \the [src].")
			...
			if("cellinstall")
				if(open && !cell)
					...
					if(istype(C))
						...
						usr.visible_message("lue [usr] inserts a power cell into \the [src].", "lue You insert the power cell into \the [src].")
